...
use super::{article_list::ArticleList, Component, reader::Reader};

pub struct FeedView<'a> {
    feed_list: ArticleList,
    feed_reader: Reader<'a>,
}

impl<'a> FeedView<'a> {
    pub fn new(feed_list: ArticleList, feed_reader: Reader<'a>) -> Self {
        Self { feed_list, feed_reader }
    }
}

impl Component for FeedView<'_> {
