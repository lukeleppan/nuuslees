...
use crate::{
    action::Action,
    components::{article_list::ArticleList, Component, feed_view::FeedView, group_view::GroupView, reader::Reader},
    config::Config,
    mode::Mode,
    tui::{Event, Frame},
};
...
impl TabViewer<'_> {
    pub fn new() -> Self {
        ...
        Self { command_tx: None, config: Config::default(), mode: Mode::GroupView, group_view, feed_view }
    }
}
...
impl Component for TabViewer<'_> {
    ...
    fn draw(&mut self, f: &mut Frame<'_>, area: Rect) -> color_eyre::Result<()> {
        match self.mode {
            Mode::Home => {}
            Mode::GroupView => {
                self.group_view.draw(f, area)?;
            }
            Mode::FeedList => {}
            Mode::ViewArticles(_) => {
                self.feed_view.draw(f, area)?;
            }
        }
        Ok(())
    }
